function [maskepi_File] = mrQ_B1FitMask_bb(dirAnts,AlignFile,SET1Fitfile,outDir,WarpAnts)
...
%  brainMask=brainMask & (Res{1}.im./(Res{2}.im*1000))< 1.7 & Res{1}.im<3000 & Res{1}.im>950 ;
%  brainMask=brainMask & (Res{1}.im./(Res{2}.im*1000))>  0.3;

% New
%brainMask=Res{2}.im>0 & Res{1}.im>0;
BM_path =   fullfile(fileparts(SET1Fitfile),'T1_SEIR_BM.nii.gz');
if ~exist(BM_path,'file')
    BM_path=fullfile(fileparts(SET1Fitfile),'T1FitNLSPR_SEIR_Dat_BrainMask_fromR1.nii.gz');
end
if ~exist(BM_path,'file') 
    error('cannot find BM file for SEIR data')
end
BM_SEIR=readFileNifti(BM_path);
brainMask=BM_SEIR.data>0 & Res{2}.im>0 & Res{1}.im>0 & (Res{1}.im./(Res{2}.im*1000))< 1.7  & Res{1}.im<3000 & Res{1}.im>950  ;
...
% brainMask=brainMask & SEIRResid<prctile(SEIRResid(brainMask),95) ;
 brainMask=brainMask & SEIRResid<prctile(SEIRResid(brainMask),99) ;
...
% 
%  brainMask = brainMask & b_fitparam>prctile(b_fitparam(brainMask_copy),5) & ...
%                          a_fitparam<prctile(a_fitparam(brainMask_copy),95) & ... 
%                          a_fitparam>prctile(a_fitparam(brainMask_copy),5)  & ...
%                          b_fitparam<prctile(b_fitparam(brainMask_copy),95) & ...
%                         WarpZ< prctile(WarpZ(brainMask_copy),98) & ...
%                         WarpZ> prctile(WarpZ(brainMask_copy),2) &  ...
%                         WarpY< prctile(WarpY(brainMask_copy),98) & ...
%                         WarpY> prctile(WarpY(brainMask_copy),2)  & ...
%                         WarpX< prctile(WarpX(brainMask_copy),98) & ...
%                         WarpX> prctile(WarpX(brainMask_copy),2)  ;
% 
%  brainMask = brainMask & b_fitparam>prctile(b_fitparam(brainMask_copy),1) & ...
%                          a_fitparam<prctile(a_fitparam(brainMask_copy),99) & ... 
%                          a_fitparam>prctile(a_fitparam(brainMask_copy),1)  & ...
%                          b_fitparam<prctile(b_fitparam(brainMask_copy),99) & ...
%                         WarpZ< prctile(WarpZ(brainMask_copy),98) & ...
%                         WarpZ> prctile(WarpZ(brainMask_copy),2)  & ...
%                         WarpY< prctile(WarpY(brainMask_copy),98) & ...
%                         WarpY> prctile(WarpY(brainMask_copy),2)  & ...
