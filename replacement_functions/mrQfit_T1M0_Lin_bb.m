function [Outputs]=mrQfit_T1M0_Lin_bb(mrQ,dataDir,outDir,B1File,MaskFile,clobber)
...
%if~( exist(t1file,'file') &&  exist(M0file,'file')  && ~clobber),
   ...
   % if notDefined('MaskFile')
        ...
        %    [brainMask,checkSlices] = mrAnatExtractBrain(M0, mmPerVox, 0.6,outDir);    %    [brainMask,checkSlices] = mrAnatExtractBrain(M0, mmPerVox, 0.5,outDir);
        [brainMask,checkSlices] = mrAnatExtractBrain(t1, mmPerVox, 0.4,outDir);    %    [brainMask,checkSlices] = mrAnatExtractBrain(M0, mmPerVox, 0.5,outDir);
